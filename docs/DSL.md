...
> **Composant** : `dsl/` · **Technologie** : Python PLY + pont pythonia · **Version** : 0.1.3 · **Date** : 7 avril 2026
...
5. [Compilateur — Validation et compilation](#5-compilateur--validation-et-compilation)
6. [Pont NestJS → Python](#6-pont-nestjs--python)
...
Le DSL de QuartierConnect est un **micro-langage de requête** qui permet aux administrateurs d'interroger les collections MongoDB sans écrire de code. Il est exposé via `POST /dsl/query` et accessible depuis le panneau d'administration React.

**Pourquoi un DSL plutôt que MongoDB directement ?**
...
- Contrôle : une liste blanche de collections, aucune opération destructive
...
flowchart LR
    ...
    E["AST dict Python<br/>type - collection - filter - limit"]
    F["Compilateur<br/>dsl/compiler.py - validation liste blanche des collections"]
    G["Requete dict MongoDB<br/>prete pour motor.find() ou motor.count_documents()"]
...
### 2.2 Intégration NestJS-Python via le pont pythonia
...
sequenceDiagram
    ...
    Svc->>Svc: Validation de longueur <= 500 caracteres
...
## 5. Compilateur — Validation et compilation

Fichier : `dsl/compiler.py`

### 5.1 Liste blanche des collections
...
| Vecteur | Mitigation |
|---------|-----------|
| Collections arbitraires | Liste blanche stricte de 5 collections |
