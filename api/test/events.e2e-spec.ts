...
describe("Events list contract (e2e)", () => {
    let app: INestApplication;
    let module: TestingModule;
    let adminToken: string;
    const token = `zzq${Date.now()}`;

    async function createEvent(title: string): Promise<void> {
    ...
    beforeAll(async () => {
        ...
        const adminEmail = `e2e-evt-admin-${Date.now()}@test.fr`;
        ...
        await createEvent(`${token}-bravo`);
        await createEvent(`${token}-alpha`);
        await createEvent(`${token}-charlie`);
    }, 60000);
    ...
    it("searches events by title across every page and reports the total", async () => {
        const paged = await request(app.getHttpServer())
            .get(`/events?search=${token}&limit=2`)
            .set("Authorization", `Bearer ${adminToken}`)
            .expect(200);
        ...
        const all = await request(app.getHttpServer())
            .get(`/events?search=${token}&limit=100`)
            .set("Authorization", `Bearer ${adminToken}`)
            .expect(200);
        expect(all.body).toHaveLength(3);
        expect(
            all.body.every((e: { title: string }) => /zzq/.test(e.title)),
        ).toBe(true);
    });

    it("sorts by title ascending when asked", async () => {
        const res = await request(app.getHttpServer())
            .get(`/events?search=${token}&sort=title&order=asc&limit=100`)
            .set("Authorization", `Bearer ${adminToken}`)
            .expect(200);
