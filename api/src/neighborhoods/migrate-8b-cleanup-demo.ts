...
async function cleanupDemo(): Promise<void> {
    ...
    try {
        ...
        const demoNeighborhoods = await neighborhoodsCol
            .find({ name: { $in: DEMO_NAMES } }, { projection: { _id: 1, name: 1 } })
            .toArray();

        if (demoNeighborhoods.length === 0) {
            process.stdout.write("No demo neighborhoods found — migration already applied (idempotent exit).\n");
            return;
        }

        const demoIds = demoNeighborhoods.map((n) => n._id.toString());
        process.stdout.write(`Found ${demoNeighborhoods.length} demo neighborhood(s): ${demoNeighborhoods.map((n) => n.name).join(", ")}\n`);

        // Step 2: collect service/event IDs to clean in Neo4j
        const demoServices = await servicesCol
            .find({ neighborhoodId: { $in: demoIds } }, { projection: { _id: 1 } })
            .toArray();
        ...
        process.stdout.write(`Services to delete: ${deletedServiceIds.length}\n`);
        ...
        const backupNeighborhoodsCol = db.collection("_backup_8b_neighborhoods");
        ...
        const existingCollections = await db.listCollections({ name: { $in: ["_backup_8b_neighborhoods", "_backup_8b_services", "_backup_8b_events"] } }).toArray();
        for (const col of existingCollections) {
            await db.dropCollection(col.name);
            process.stdout.write(`Dropped existing backup: ${col.name}\n`);
        }

        await neighborhoodsCol.aggregate([
            { $match: { name: { $in: DEMO_NAMES } } },
            { $out: "_backup_8b_neighborhoods" },
        ]).toArray();

        const backupNbCount = await backupNeighborhoodsCol.countDocuments();
        process.stdout.write(`Backed up ${backupNbCount} neighborhood(s) → _backup_8b_neighborhoods\n`);

        if (deletedServiceIds.length > 0) {
            await servicesCol.aggregate([
        ...
        process.stdout.write(`Backed up ${backupSvcCount} service(s) → _backup_8b_services\n`);

        if (deletedEventIds.length > 0) {
            await eventsCol.aggregate([
                { $match: { neighborhoodId: { $in: demoIds } } },
        ...
        process.stdout.write(`Backed up ${backupEvtCount} event(s) → _backup_8b_events\n`);

        if (backupNbCount !== demoNeighborhoods.length) {
            throw new Error(`Backup count mismatch: expected ${demoNeighborhoods.length} neighborhoods, got ${backupNbCount}. Aborting.`);
        }

        // Step 4: delete from Mongo
        process.stdout.write("\n--- Deleting from MongoDB ---\n");

        const svcResult = await servicesCol.deleteMany({ neighborhoodId: { $in: demoIds } });
        process.stdout.write(`Deleted ${svcResult.deletedCount} service(s)\n`);

        const evtResult = await eventsCol.deleteMany({ neighborhoodId: { $in: demoIds } });
        process.stdout.write(`Deleted ${evtResult.deletedCount} event(s)\n`);

        const nbResult = await neighborhoodsCol.deleteMany({ name: { $in: DEMO_NAMES } });
        process.stdout.write(`Deleted ${nbResult.deletedCount} neighborhood(s)\n`);
        ...
        try {
            ...
            process.stdout.write(`Deleted ${nbCleanup.records[0]?.get("deleted") ?? 0} Neighborhood node(s)\n`);
            ...
            if (deletedServiceIds.length > 0) {
                ...
                process.stdout.write(`Deleted ${svcCleanup.records[0]?.get("deleted") ?? 0} Service node(s)\n`);
            }
            ...
            if (deletedEventIds.length > 0) {
                ...
                process.stdout.write(`Deleted ${evtCleanup.records[0]?.get("deleted") ?? 0} Event node(s)\n`);
            }
        } finally {
            await session.close();
            await driver.close();
