...
function normalizeTotpSecret(email: string): void {
  try {
    pgQuery(`UPDATE users SET totp_secret='${DEMO_TOTP_SECRET}' WHERE email='${email}'`);
  } catch {
    console.warn(`  ! Could not normalize TOTP secret for ${email} — is Docker running?`);
  }
}
...
async function seedAccount(email: string, role: string): Promise<void> {
  ...
  const registerRes = await post('/auth/register', { email, password: DEMO_PASSWORD });
